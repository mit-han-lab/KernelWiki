"""
NVFP4 Block-Scaled GEMV - Attempt 11

╔══════════════════════════════════════════════════════════════════════════════╗
║  ⚖️ EQUIVALENT - Register Count Tuning (-maxrregcount=64)                     ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  Performance Results (vs Attempt 7 baseline with 80 regs):                    ║
║    M=7168, K=16384, L=1  →  26.7 µs  (0% difference)                          ║
║    M=4096, K=7168,  L=8  →  45.1 µs  (0% difference)                          ║
║    M=7168, K=2048,  L=4  →  16.4 µs  (0% difference)                          ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  What was tested:                                                             ║
║    - Reduced max register count from 80 to 64                                 ║
║    - Goal: Increase occupancy by reducing register pressure                   ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  Analysis:                                                                    ║
║    - Kernel likely uses <64 registers naturally                               ║
║    - The limit has no effect since we're already under it                     ║
║    - Both 64 and 80 produce identical code                                    ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  Also tested (separately): ROWS_PER_BLOCK Tuning (BLOCK_SIZE=256, RPB=8)      ║
║    - Result: ~0% difference (within measurement noise)                        ║
║    - Conclusion: Block size tuning (128→256, RPB 4→8) has no measurable       ║
║      impact as long as 32 threads/row is maintained                           ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  Also tested: #pragma unroll 8 (vs baseline unroll 4)                         ║
║    - Results: -5% to -87% regression!                                         ║
║    - Why: Register pressure → spilling, I-cache misses, wasted instructions   ║
║    - Conclusion: #pragma unroll 4 is optimal. Do NOT increase unroll factor.  ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
#define THREADS_PER_ROW (BLOCK_SIZE / ROWS_PER_BLOCK)  // 32 threads per row
...
__global__ void gemv_nvfp4_kernel(
    ...
    int l = blockIdx.y;
    int tid = threadIdx.x;
    int row_in_block = tid / THREADS_PER_ROW;  // 0-7
    int tid_in_row = tid % THREADS_PER_ROW;    // 0-31
    ...
    // 32 threads = 1 warp, perfect for shuffle reduction
    const int lane = tid_in_row;
    float row_sum = acc;

    // Warp reduction for 32 threads
